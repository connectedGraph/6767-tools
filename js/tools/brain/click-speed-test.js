  ...
  methods: {
    ...
    areaClick(clickEvent) {
      ...
      if (this.gameState == 0) {
        ...
        divElement.style.left = clickEvent.offsetX - 50 + "px";
        divElement.style.top = clickEvent.offsetY - 50 + "px";
        document.querySelector(".game-area").appendChild(divElement);
        setTimeout(function () {
          divElement.remove();
        }, 600);
      }
    ...
    setTime() {
      if (this.time >= this.gameDuration * 10) {
        ...
        if (this.score < this.gameDuration * 100 && !this.usedCheat) {
          ...
          c.game.addRankingList(gameInfoPayload);
        }
      } else {
        this.time++;
      }
      var secondsInt = parseInt(this.time / 10, 10);
      var timeRemainder = this.time % 10;
      this.timeStr = secondsInt + "." + timeRemainder;
    },
