Vue.createApp({
  ...
  mounted() {
    // 支持 /woman-clothing-size?height=160&weight=50 这类带参链接进入
  ...
  methods: {
    refreshResult() {
      var clothingSizeValue = this.computeBraSize();
      if (clothingSizeValue != null) {
        this.computeResult = locales.computeResult.replace("XXXL", "<span>" + clothingSizeValue + "</span>");
      } else {
    ...
    computeBraSize() {
      ...
      if (c.isNumber(this.formData.height) && c.isNumber(this.formData.weight)) {
        var sizeTable = ["S", 80, 99, 145, 149, "S", 90, 99, 150, 169, "S", 80, 89, 160, 169, "M", 80, 99, 170, 172, "M", 90, 99, 173, 175, "M", 100, 109, 145, 149, "M", 100, 109, 155, 179, "L", 110, 119, 145, 149, "L", 110, 119, 155, 179, "L", 120, 129, 155, 159, "L", 120, 129, 170, 179, "XS", 80, 89, 150, 159, "XL", 110, 119, 150, 154, "XL", 120, 129, 145, 154, "XL", 120, 129, 160, 169, "XL", 130, 139, 160, 179, "XL", 140, 149, 173, 179, "XXL", 130, 139, 150, 159, "XXL", 140, 149, 155, 172];
        for (var sizeIndex = 0; sizeIndex < sizeTable.length / 5; sizeIndex++) {
          if (this.formData.weight * 2 >= sizeTable[sizeIndex * 5 + 1] && this.formData.weight * 2 <= sizeTable[sizeIndex * 5 + 2] && this.formData.height >= sizeTable[sizeIndex * 5 + 3] && this.formData.height <= sizeTable[sizeIndex * 5 + 4]) {
            matchedSize = sizeTable[sizeIndex * 5];
            break;
          }
        }
        if (matchedSize == null) {
