(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["gifParserPlugin"] = factory();
	else
		root["gifParserPlugin"] = factory();
...
// CONCATENATED MODULE: ./src/lib/GetBitArray.ts
// 获取二进制数组
function getBitArray(val) {
  var bits = [];
...
// CONCATENATED MODULE: ./src/lib/GetPaletteSize.ts
...
function getFrameDuration(duration) {
...
// CONCATENATED MODULE: ./src/model/FrameImage.ts


var FrameImage_FrameImage = /*#__PURE__*/_createClass(function FrameImage() {
  _classCallCheck(this, FrameImage);
...
// CONCATENATED MODULE: ./src/lib/ReadSubBlock.ts
// 读取GIF的子数据块
function readSubBlock(view, pos, read) {
  ...
  while (true) {
    var size = view.getUint8(pos + subBlock.size);
...
function byteToString(view, length, byteOffset) {
  var value = "";

  for (var i = 0; i < length; ++i) {
    var _char = view.getUint8(byteOffset + i);

    value += String.fromCharCode(_char > 127 ? 65533 : _char);
  }

  return value;
}
// CONCATENATED MODULE: ./src/lib/GetGlobalPalette.ts
...
function getGlobalPaletteInfo(dataView, gifInfo, pos) {
  var PaletteColorsRGB = []; // 解析全局调色板

  var unpackedField = getBitArray(dataView.getUint8(10));

  if (unpackedField[0]) {
    var globalPaletteSize = getPaletteSize(unpackedField);
    gifInfo.globalPalette = true;
    gifInfo.globalPaletteSize = globalPaletteSize / 3; // 调整指针位置

    pos += globalPaletteSize;

    for (var i = 0; i < gifInfo.globalPaletteSize; i++) {
      var palettePos = 13 + i * 3;
      var r = dataView.getUint8(palettePos);
      var g = dataView.getUint8(palettePos + 1);
      var b = dataView.getUint8(palettePos + 2);
      PaletteColorsRGB.push({
        r: r,
        g: g,
        b: b
      });
  ...
  return {
    pos: pos,
    PaletteColorsRGB: PaletteColorsRGB
  };
}
...
var main_GifParser = /*#__PURE__*/function () {
  function GifParser(url) {
    var _this = this;

    _classCallCheck(this, GifParser);

    this.urlLoadStatus = undefined; // 当前指向DataView的指针位置
    ...
    if (url) {
      this.urlLoadStatus = false; // 解析url，将其转化为DataView格式的数据
  ...
  _createClass(GifParser, [{
    key: "getInfo",
    value: function () {
      var _getInfo = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(gifStream) {
        var _getGlobalPaletteInfo, pos, PaletteColorsRGB, frame, block, unpackedField, localPaletteSize, subBlock;
        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                _context.next = 2;
                return this.validityCheck(gifStream);
              ...
              case 4:
                if (!(this.dataView.getUint16(0) != 0x4749 || this.dataView.getUint16(2) != 0x4638)) {
                  _context.next = 6;
                  break;
                }

                return _context.abrupt("return", this.gifInfo);

              case 6:
                // 经过上述判断后，此时的GIF已经有效了
                this.gifInfo.valid = true; // 获取GIF图像的宽，高

                this.gifInfo.width = this.dataView.getUint16(6, true);
                this.gifInfo.height = this.dataView.getUint16(8, true); // 解析全局调色板

                _getGlobalPaletteInfo = getGlobalPaletteInfo(this.dataView, this.gifInfo, this.pos), pos = _getGlobalPaletteInfo.pos, PaletteColorsRGB = _getGlobalPaletteInfo.PaletteColorsRGB; // 修改指针位置

                this.pos = pos; // 设置全局调色板

                this.gifInfo.globalPaletteColorsRGB = PaletteColorsRGB; // gif每一帧的图片描述

                frame = new FrameImage_FrameImage(); // 循环读取dataView, 读取gif每一帧的数据信息

              case 13:
                if (false) {}

                _context.prev = 14;
                block = this.dataView.getUint8(this.pos);
                _context.t0 = block;
                _context.next = _context.t0 === 0x21 ? 19 : _context.t0 === 0x2c ? 21 : _context.t0 === 0x3b ? 37 : 38;
                break;
              ...
              case 21:
                ...
                frame.imageInfo.height = this.dataView.getUint16(this.pos + 7, true);
                unpackedField = getBitArray(this.dataView.getUint8(this.pos + 9));
                ...
                this.index++; // 重置frame，继续下一轮解析

                frame = new FrameImage_FrameImage();
                frame.imageInfo.identifier = this.index.toString(); // 判断当前GIF是否可以动
                ...
                this.pos += 11;
                subBlock = readSubBlock(this.dataView, this.pos, false);
    ...
    key: "parseGraphicsControlBlock",
    value: function parseGraphicsControlBlock(dataView, frame) {
      var type = dataView.getUint8(this.pos + 1); // 图形控制块

      if (type === 0xf9) {
        var length = dataView.getUint8(this.pos + 2);

        if (length === 4) {
          var delay = getFrameDuration(dataView.getUint16(this.pos + 4, true));
          frame.imageInfo.delay = delay; // gif总时长自增

          this.gifInfo.duration += delay;
          var unpackedField = getBitArray(dataView.getUint8(this.pos + 3));
      ...
      this.pos += 2; // 获取子模块，做进一步解析

      var subBlock = readSubBlock(dataView, this.pos, true);

      switch (type) {
        // 应用扩展块数据
        case 0xff:
          var identifier = byteToString(dataView, 8, this.pos + 1); // 可能会出现多个应用程序扩展块，我们需要确保仅在标识符为 NETSCAPE 时处理GIF循环次数
    ...
    key: "validityCheck",
    value: function () {
      var _validityCheck = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(gifStream) {
        var loadResult, gifBuffer;
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                ...
                _context2.next = 3;
                return this.gifLoadStatus();

              case 3:
                loadResult = _context2.sent;
              ...
              case 6:
                ...
                _context2.next = 9;
                return gifStream.arrayBuffer();

              case 9:
                gifBuffer = _context2.sent;

                if (!(gifBuffer.byteLength < 10)) {
                  _context2.next = 12;
                  break;
                }

                return _context2.abrupt("return", this.gifInfo);
      ...
      function validityCheck(_x2) {
        return _validityCheck.apply(this, arguments);
      }
...
/* harmony default export */ var entry_lib = __webpack_exports__["default"] = (main_GifParser);
